...
foreach($orgs as $org)
{
	...
	foreach($classs as $class)
	{
	$countinfo=classOrgHasObject($class[0],$org[0]);
	if($countinfo[0]==0)
...
function showclasstable($classid)
{
	...
	$countinfo=classOrgHasObject($classid,$org[0]);
	if($countinfo[0])
...
function showOrgs($hascustomer)
{
	...
	$query="select * from tbOrg where id in(select fdOrgID from tbMember where fdRoleID!=6)";
	$result=mysql_query($query);
	while($row=mysql_fetch_assoc($result))
	{
...
//每个组织含有的每类物品数目及其状态；
function classOrgHasObject($classid,$orgid)
{
	$result_0=0;//总数
	$result_1=0;//在途出
	$result_2=0;//在途入
...
function echoxmlOCO($orgid)
{
	...
	for($i=0;$i<count($sorts);$i++)
	{
		...
		for($j=0;$j<count($classs);$j++)
		{
				...
				$data=classOrgHasObject($classs[$j][0],$orgid);
...
function echoxmlOOC($class)
{
	...
	for($i=0;$i<count($orgs);$i++)
	{
		$org=$xml->addChild("org");
		$org->addAttribute("name",$orgs[$i][1]);
	    $data=classOrgHasObject($class[1],$orgid[$i][0]);
...
{
	echo "<select class='type roleshow' >";
